...
export const updateUser = (req, res) => {
  ...
  jwt.verify(token, "secretkey", (err, userInfo) => {
    ...
    //ako je lozinka poslata, dodamo je nakon što je šifriramo
    if(req.body.password) {
      ...
      values.splice(1, 0, hashedPassword); //ubacimo šifriranu lozinku na drugo mjesto
    };
  ...
  jwt.verify(token, "secretkey", (err, userInfo) => {
    ...
    db.query(q, [userId], (err, result) => {
      ...
      if (result.affectedRows > 0) {
        ...
        res.status(404).json("Korisnik nije pronađen.");
      }
    });
  });
};
