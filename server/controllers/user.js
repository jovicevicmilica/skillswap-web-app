...
export const updateUser = (req, res) => {
  ...
  jwt.verify(token, "secretkey", (err, userInfo) => {
    ...
    db.query(q, [req.body.email, req.body.password, req.body.name, req.body.town, req.body.profilePic, req.body.coverPic, req.body.primarySkill, req.body.primarySkillLevel, req.body.learningPref, userInfo.id], (err, data) => {
        if(err) return res.status(500).json(err);
        if(data.affectedRows > 0) {
          ...
          db.query(q2, [userInfo.id], (err, updatedUserData) => {
            if (err) return res.status(500).json(err);
            const { password, ...updatedUserInfo } = updatedUserData[0];
